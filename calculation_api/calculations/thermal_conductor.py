...
# Temperatures (K) — tüm malzemeler için ortak
TEMPERATURES: List[float] = [
    0.5,1,2,4,6,8,10,15,20,25,30,35,40,50,60,70,80,90,100,120,140,160,180,200,250,300,350,400,450,500
]

# Malzeme listesi (JS’den birebir taşındı)
...
def _interpolated_integ_value(cd: List[float], td: List[float], t: float, n: int) -> float:
    """Quadratic interpolation of integral values (JS'deki 'interpolatedIntegValue')."""
    # n, n-1, n+1 indislerine göre üç nokta interpolasyonu
    return (
...
def _calc_cond_integral(material: ThermCondMaterial, t: float) -> float:
    """JS'deki calcCondIntegral fonksiyonunun Python karşılığı."""
    cd = material.conductivity_pts
    td = TEMPERATURES

    # en yakın alt sıcaklık noktasını bul (td sıralı: düşük -> yüksek)
    i = 0
    ...
    # Ağırlıklı ortalama: (n..n+2) ile (n-1..n+1) pencerelerinin birleşimi
    num = (
...
def _area_over_length_cm(
    ...
    *,
    ...
    """
    A/L değerini (alan/uzunluk) cm biriminde döndürür.
    JS mantığı: giriş birimi inç ise sonuç numeric olarak cm’ye çevrilir.
    Uyarılar listesine duvar kalınlığı düzeltmesi gibi notlar eklenir.
    """
    warnings: List[str] = []

    # Birim dönüşümü: in -> cm
    k = 2.54 if units == "in" else 1.0
    ...
    if shape == "rod":
        ...
        d_cm = diameter * k
        a_over_l = 3.141592653589793 / 4.0 * d_cm * d_cm / L_cm
        ...
        # JS formülü: π * wall * (diam - wall) / length
        a_over_l = 3.141592653589793 * w_cm * (d_cm - w_cm) / L_cm
...
def compute_heat_load(
    *,
    ...
    """
    Orijinal JS 'calculate()' fonksiyonunun saf hesap versiyonu.
    Dönüş:
      {
        "heat_load_watts": float,
        "area_over_length_cm": float,
        "warnings": [str, ...]
      }
    """
    ...
    if temp_lo > temp_hi:
        # Orijinal JS bunu engellemiyordu; burada uyarı yerine swap etme de yapılabilir.
        # Şimdilik hata verelim.
        raise ValueError("Low temperature must be <= high temperature.")

    mat = _get_material(material)

    a_over_l_cm, warnings = _area_over_length_cm(
        shape, units,
        length=length, diameter=diameter, wall=wall, width=width, height=height
    )

    # Isı sızıntısı: (Integral(hi) - Integral(lo)) * (A/L)
    # (JS’deki calcCondIntegral ile aynı)
    hi_int = _calc_cond_integral(mat, temp_hi)
    lo_int = _calc_cond_integral(mat, temp_lo)
    heat_leak_watts = (hi_int - lo_int) * a_over_l_cm

    return {
        "heat_load_watts": float(f"{heat_leak_watts:.6g}"),  # sayıyı temiz döndür
        "area_over_length_cm": a_over_l_cm,
        "warnings": warnings
    }
